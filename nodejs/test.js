...
worker.on('message', (msg) => {
  if ((msg.type == 'cardList-get' && !getList) || msg.type == 'cardList-update') {
    // 收到cardlist的get或者update的时候更新nodejs的cardlist
    getList = true;
  ...
  } else if (msg.type == 'dunInfo-update' && !getList) {
    // 第一次获取信息后，获取cardList
    getCardList();
  }
});
...
http
  .createServer((req, res) => {
    ...
    if (urlObj.pathname == '/canteen/cardList') {
      ...
      res.end();
      userCardList = {};
    } else if (urlObj.pathname == '/canteen/userNumber') {
      // 获取用户总数量
...
// 建立与3001端口连接
https
  .createServer(httpsOption, (req, res) => {
    // json文件 utf-8解析及写入cardList
    let urlObj = urlib.parse(req.url, true);
    // 判断路径是否正确
    if (urlObj.pathname == '/canteen/cardList') {
