...
export class ArknightsOfficialWebDataSource extends DataSource {
  ...
  async processData(rawDataText) {
    ...
    let $items = $(
      ".articleList[data-category-key='ANNOUNCEMENT'] .articleItem,.articleList[data-category-key='ACTIVITY'] .articleItem,.articleList[data-category-key='NEWS'] .articleItem",
      $(rawDataText)
    );
    $items.each((i, e) => {
      const $item = $(e);
