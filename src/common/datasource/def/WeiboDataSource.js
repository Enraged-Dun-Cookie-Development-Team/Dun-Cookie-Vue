...
export class WeiboDataSource extends DataSource {
    ...
    async processData(rawDataText) {
      ...
      data.data.cards.length > 0
            ...
            data.data.cards.forEach((x) => {
                // 是否显示转发内容
                if (
                    !Settings.dun.showRetweet &&
          x.hasOwnProperty("mblog") &&
          x.mblog.hasOwnProperty("retweeted_status")
                ...
                if (x.hasOwnProperty("mblog")) {
                    ...
                    let weiboId =
            containerId.substring(containerId.length - 10, containerId.length) +
            "/" +
            x.mblog.bid;
                    ...
                    const builder = DataItem.builder(this.dataName)
                        ...
                        .timeForDisplay(
                            TimeUtil.format(new Date(time), "yyyy-MM-dd hh:mm:ss")
                        )
                        .content(
                ...
                dynamicInfo.text
                    ...
                    .replace(/<br \/>/g, "\n")
                        ...
                        .previewList(
                            dynamicInfo.pic_ids &&
                dynamicInfo.pic_ids.map(
                    (x) => `https://wx1.sinaimg.cn/thumbnail/${x}`
                )
                        )
                        .imageList(
                            dynamicInfo.pic_ids &&
                dynamicInfo.pic_ids.map(
                    (x) => `https://wx1.sinaimg.cn/large/${x}`
                )
                        )
                        .imageHttpList(
                            dynamicInfo.pic_ids &&
                dynamicInfo.pic_ids.map(
                    (x) => `http://wx3.sinaimg.cn/large/${x}`
                )
                        );
   ...
   */
    ...
    static async fetchUserInfo(uid) {
        ...
        return new UserInfo(
            json.data.userInfo.screen_name + "微博",
            json.data.userInfo.avatar_hd
        );
    }
}
