    ...
    async createWithSpecialIcon(id, iconUrl, title, message, imageUrl) {
        ...
        if (typeof imageUrl === 'string' && imageUrl.startsWith("http")) {
            try {
              ...
              DebugUtil.debugConsoleOutput(0, 'debug', '%c 推送图片 ', 'color: #eee; background: #e5a335', `推送图片响应：`, response);
              const blob = await response.blob();

              objectUrl = URL.createObjectURL(blob);
            } catch (e) {
              DebugUtil.debugConsoleOutput(0, 'error', '%c 推送图片 ', 'color: #eee; background: #e53935', `推送图片报错：`, e);
              objectUrl = undefined;
              imageUrl = undefined;
            }
        }
