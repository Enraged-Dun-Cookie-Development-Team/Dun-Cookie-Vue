...
class HttpUtil {
   ...
   * @param {boolean} logError 是否打印异常日志，如果关闭的话需要自行确保不会丢失异常信息
   * @return {Promise}
   */
  static async GET_Json(url, options = {}, failController, logError = true) {
    const response = await HttpUtil.GET(url, options, failController, logError);
    if (response) {
   ...
   * @return {Promise}
   */
  static async GET(url, options = {}, failController, logError = true) {
    ...
    try {
      ...
      if (failController) {
        if (e instanceof RequestError) {
          return failController(e);
        } else {
          return failController(
            new RequestError(`平台的请求方法抛出的异常不是RequestError：${String(e)}`, undefined, e)
          );
        }
      }
    }
