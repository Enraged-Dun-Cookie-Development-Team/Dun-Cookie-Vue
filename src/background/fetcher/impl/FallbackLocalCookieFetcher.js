 ...
 * @param serverInfo
 * @return {Promise<FetchControllerConfig>}
 */
async function _buildConfig(fetchConfig, serverInfo) {
  ...
  // fallback的默认间隔强行改为至少60秒，以避免过多的请求
...
export class FallbackLocalCookieFetcher extends CustomLocalCookieFetcher {
  ...
  async _checkAvailable(fetchConfig) {
    ...
    const config = await _buildConfig(fetchConfig, serverInfo);
    try {
