    ...
    copyData(item) {
      ...
      setTimeout(() => {
        new Promise((resolve, reject) => {
          new QRCode.toCanvas(item.jumpUrl).then((canvas) => {
            ...
            return new Promise((resolve, reject) => {
              return html2canvas(
                document.querySelector(
                  `.wrapper[data-id='${item.id}'] .wrapper-content`
                )
          ...
          .then(({ textCanvas, item, data }) => {
            ...
            if (data.headFigure != undefined) {
              canvasWidth =
                textCanvas.width > data.headFigure.width
                  ? textCanvas.width
                  : data.headFigure.width;
            ...
            if (data.headFigure) {
              ctx.drawImage(
                data.headFigure,
                (canvas.width - data.headFigure.width) / 2,
                textCanvas.height + 160,
                data.headFigure.width,
                data.headFigure.height
              );
            }
