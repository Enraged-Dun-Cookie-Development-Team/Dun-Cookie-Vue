<template>
  <div
    id="timeline-area" ref="totalScrollArea"
    :class="settings.display.announcementScroll && timelineEnableScroll ? 'scrollTimeline' : ''"
  >
    <Search
      ref="SearchModel" :search-show="searchShow"
      @searchTextChange="changeFilterText"
    />
    <el-card
      v-loading="loading" shadow="never" class="info-card online-speak"
      :class="searchShow ? 'searching' : ''"
      element-loading-text="【如果你看到这条信息超过1分钟，去*龙门粗口*看看网络有没有*龙门粗口*正常连接】"
    >
      <div
        class="announcement-area" @wheel="gowheel" @mouseover="mouseOverAnnouncement"
        @mouseleave="mouseLeaveAnnouncement"
      >
        <el-carousel
          v-if="!loading" ref="swiper" height="100px"
          arrow="never" direction="vertical" :interval="3000"
          :autoplay="true"
        >
            ...
            <div class="day-info">
              <div class="day-info-content">
                <div class="day-info-content-top">
                  <div>
                    <div
                      v-for="(item, index) in onlineDayInfo.countdown" :key="index"
                      class="day-info-content-top-card-area"
                    >
                      <div>
                        距离
                        <el-tooltip
                          v-if="item.remark" :content="item.remark"
                          placement="right"
                        >
                  ...
                  <div
                    v-if="settings.feature.san && imgShow" class="sane-area"
                ...
                <div class="day-info-content-bottom">
                  <div class="day-info-content-bottom-card-area">
                    <el-tooltip
                      v-for="item in dayInfo" :key="item.type"
    ...
    <el-tabs
      v-if="settings.display.showByTag" v-model="currentTag"
      :stretch="true" @tab-click="selectListByTag"
    >
      <el-tab-pane
        v-for="item of transformToSortList(cardListByTag)" :key="item.dataName" :label="item.dataName"
        :name="item.dataName"
      >
        <span slot="label">
          <el-tooltip
            effect="dark" :content="getDataSourceByName(item.dataName).title"
            placement="top"
          >
    ...
    <el-timeline
      v-if="LazyLoaded" ref="elTimelineArea" :class="[
    ...
    >
      ...
      >
        ...
        <span class="card-btn-area">
          <el-button
            class="to-copy-share" :class="{ 'special-source': item.componentData }" size="small"
            title="左键生成图片分享，右键九宫格分享" @click="copyData(item)" @contextmenu.prevent.native="rightCopyData(item)"
          >
          ...
          <el-button
            v-if="!item.componentData" class="to-url-btn" size="small"
            title="前往该条内容"
        ...
        >
          <component
            :is="resolveComponent(item)" :item="item"
            :show-image="imgShow"
          />
        </el-card>
      </MyElTimelineItem>
    ...
    <div
      ...
      style="height: 300px" element-loading-custom-class="page-loading"
    ></div>
    <el-dialog
      :modal-append-to-body="false" title="图片自动复制出错，请于图片右键复制图片"
      :visible.sync="imageError" width="80%"
    >
